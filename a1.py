...
class GameBoard:
    ...
    # _occupied_dict:
    #   dict of occupied squares on the board
    ...
    _occupied_dict: dict[Tuple[int, int], []]
    _grid: str
    ...
    def place_character(self, c: Character) -> None:
        ...
        """
        self
        if not self._occupied_dict[(c.x, c.y)]:
            self._occupied_dict[(c.x, c.y)] = []
            self._occupied_dict[(c.x, c.y)].append(c)

        # TODO Task #1
        # if ((c.x, c.y) not in self._occupied_dict) or
    ...
    def at(self, x: int, y: int) -> List[Character]:
        ...
        True
        """
        if ((x,y) not in self._occupied_dict) or not self._occupied_dict:
            return []
        else:
            return self._occupied_dict[(x, y)]
    ...
    def to_grid(self) -> List[List[chr]]:
        ...
        [['P', '-', '-'], ['-', 'R', 'C']]
        """
        return_lst = self.grid_init()
        for occ in self._occupied_dict:
            if isinstance(occ, Raccoon):
                return_lst[occ.x][occ.y] = 'R'
            elif isinstance(occ, SmartRaccoon):
                return_lst[occ.x][occ.y] = 'S'
            elif isinstance(occ, Player):
                return_lst[occ.x][occ.y] = 'P'
            elif isinstance(occ, GarbageCan):
                return_lst[occ.x][occ.y] = 'C'
            # elif isinstance(occ, SmartRaccoon):
            #     return_lst[occ.x][occ.y] = 'S'
            elif isinstance(occ, RecyclingBin):
                return_lst[occ.x][occ.y] = 'B'
            elif isinstance(occ, SmartRaccoon):
                return_lst[occ.x][occ.y] = '@'
        return return_lst

    def grid_init(self)-> List[List[chr]]:
        """init a blank grid the width and height of self
        """
        return_lst = []
